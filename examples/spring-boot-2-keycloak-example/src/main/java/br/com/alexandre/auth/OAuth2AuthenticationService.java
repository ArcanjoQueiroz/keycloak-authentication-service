...
@Service
@Profile("!test")
public class OAuth2AuthenticationService extends AbstractAuthenticationService implements AuthenticationService {
    ...
    protected User decode(final OAuth2AuthenticationDetails authDetails) {
        ...
        final String userName = claims.get("preferred_username") != null ? claims.get("preferred_username").toString() : "unknown";        
        final String givenName = claims.get("given_name") != null ? claims.get("given_name").toString() : null;        
        final String familyName = claims.get("family_name") != null ? claims.get("family_name").toString() : null;
        final String email = claims.get("email") != null ? claims.get("email").toString() : null;
        
        final List<String> roles = claims.get("resource_access") != null ? 
